...
import type {
  ...
  String3DCustomFilterImplementation,
} from "../core/filters/String3DCustomFilter";
import type { String3DFilterChain } from "../core/filters/String3DFilterTypes";
...
export class ThreePostProcessPipeline implements I3DPostProcessPipelineRuntime {
  private readonly backend: I3DBackend;
  private engine: I3DEngine;
  private renderer: I3DRenderer;
  private runtime: I3DPostProcessRuntime;
  private customFilterRegistry: I3DCustomFilterRegistryRuntime;
  ...
  constructor(context: {
    ...
    const { engine, renderer, runtime, width, height, customFilterRegistry } = context;
    this.backend = engine.getBackend?.() || "webgl";
    this.engine = engine;
    this.renderer = renderer;
    this.runtime = runtime;
    this.customFilterRegistry = customFilterRegistry;
    this.width = width;
    this.height = height;
    ...
    const create = (w: number, h: number) => this.runtime.createRenderTarget(w, h);
    this.pool = new RenderTargetPool(create);
  }

  public isSupported(): boolean {
    return this.runtime.isSupported(this.renderer);
  }
  ...
  private renderPass(
    ...
    this.runtime.setRenderTarget(this.renderer, output);
    ...
    if (clear) {
      this.runtime.clear(this.renderer, true, true, true);
    }
  ...
  private createShaderMaterial(params: any): I3DMaterial {
    return this.runtime.createShaderMaterial(params);
  }
  ...
  private getCustomMaterial(name: string): I3DMaterial | null {
    ...
    const def = this.customFilterRegistry.get(normalized) as
      | String3DCustomFilterDefinition
      | undefined;
    if (!def) return null;
    const impl = this.resolveCustomFilterImplementation(def);
    if (!impl) return null;
    const shaders = this.resolveCustomFilterShaders(impl);
  ...
  private resolveCustomFilterImplementation(
    definition: String3DCustomFilterDefinition,
  ): String3DCustomFilterImplementation | undefined {
    const byBackend =
      definition.implementations?.[this.backend] || definition.implementations?.custom;
    if (byBackend) return byBackend;
    if (definition.fragmentShader && this.backend === "webgl") {
      return {
        kind: "shader",
        language: "glsl",
        stage: "fragment",
        code: definition.fragmentShader,
      };
    }
    return undefined;
  }

  private resolveCustomFilterShaders(
    implementation: String3DCustomFilterImplementation,
  ): { vertexShader: string; fragmentShader: string } | null {
    if (implementation.kind !== "shader") return null;
    if (implementation.language !== "glsl" && implementation.language !== "custom") return null;
    if (implementation.stage && implementation.stage !== "fragment") return null;
    return {
      vertexShader: implementation.vertexCode || this.getVertexShader(),
      fragmentShader: implementation.code,
    };
  }
